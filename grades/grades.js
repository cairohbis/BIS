...
(function () {
  ...
  ───────────────────────────────────────── */
  let _state = {
    screen:"home", allRecords:null, expandedYears:{}, editing:false, indexQuery:"", indexSort:"name",
  ...
  /* ─────────────────────────────────────────
     إضافة — سجل المواد الشامل (ترتيب بالاسم)
     يبني فهرسًا لكل مادة سبق تسجيلها في أي سجل/ترم
     تلقائيًا من البيانات المخزّنة أصلاً — بدون أي
     إدخال أو تسجيل إضافي من الطالب.
  ───────────────────────────────────────── */
  function _buildSubjectIndex(records) {
    const order = [];
    YEARS.forEach(y=>{ TERMS.forEach(t=>{ order.push(y+"__"+t); }); });
    const map = new Map();
    records.forEach(r=>{
      (r.subjects||[]).forEach(s=>{
        if (!s.name || !s.grade) return;
        const key = s.name.trim().replace(/\s+/g," ");
        if (!key) return;
        if (!map.has(key)) map.set(key,{ name:key, attempts:[] });
        map.get(key).attempts.push({
          year:r.year, term:r.term, grade:s.grade,
          mid:GRADE_MAP[s.grade]?.mid||0,
          order:order.indexOf(_key(r.year,r.term)),
        });
      });
    });
    const list = Array.from(map.values());
    list.forEach(g=>{
      g.attempts.sort((a,b)=>b.order-a.order); // الأحدث أولاً
      g.best = Math.max(...g.attempts.map(a=>a.mid));
      g.latest = g.attempts[0];
    });
    return list;
  }

  function _idxCardHtml(g) {
    const bestColor = _perfLabel(g.best).color;
    const attemptsHtml = g.attempts.map(a=>`
      <div class="gr-idx-attempt">
        <span class="gr-idx-attempt-meta"><i class="fa-solid fa-calendar-days"></i> ${_escHtml(a.year)} · ${_escHtml(a.term)}</span>
        ${_gradeTag(a.grade)}
      </div>`).join("");
    return `<div class="gr-idx-card">
      <div class="gr-idx-head">
        <div class="gr-idx-name"><i class="fa-solid fa-book-open"></i> ${_escHtml(g.name)}</div>
        ${g.attempts.length>1?`<div class="gr-idx-count">${g.attempts.length} محاولات</div>`:""}
      </div>
      <div class="gr-idx-attempts">${attemptsHtml}</div>
      <div class="gr-bar-row">
        <div class="gr-bar-label">أفضل نتيجة</div>
        <div class="gr-bar-track"><div class="gr-bar-fill" style="width:${g.best}%;background:${bestColor}"></div></div>
        <div class="gr-bar-val" style="color:${bestColor}">${g.best}%</div>
      </div>
    </div>`;
  }

  function _sortIndexList(list, mode) {
    const arr = list.slice();
    if (mode==="best-desc") arr.sort((a,b)=> b.best-a.best || a.name.localeCompare(b.name,"ar"));
    else if (mode==="best-asc") arr.sort((a,b)=> a.best-b.best || a.name.localeCompare(b.name,"ar"));
    else arr.sort((a,b)=>a.name.localeCompare(b.name,"ar"));
    return arr;
  }

  function _idxSortChipsHtml() {
    const sort = _state.indexSort||"name";
    const opts = [
      { m:"name",      icon:"fa-arrow-down-a-z",        label:"أبجدي" },
      { m:"best-desc", icon:"fa-arrow-down-wide-short",  label:"الأعلى تقديرًا" },
      { m:"best-asc",  icon:"fa-arrow-up-short-wide",    label:"الأدنى تقديرًا" },
    ];
    return `<div class="grades-chips" id="gr-idx-sort-chips">
      ${opts.map(o=>`<button class="grades-chip ${sort===o.m?"active":""}"
        onclick="window.GradesModule._setIndexSort('${o.m}',this)">
        <i class="fa-solid ${o.icon}"></i> ${o.label}
      </button>`).join("")}
    </div>`;
  }

  function _idxListHtml(list, totalCount) {
    return `
      <div class="grades-section-label">
        <i class="fa-solid fa-list-ol"></i> المواد مرتبة بالاسم
        <span class="grades-arc-total">${list.length} من ${totalCount}</span>
      </div>
      <div class="gr-idx-list">
        ${list.length ? list.map(_idxCardHtml).join("") : `<div class="gr-nodata">لا توجد نتائج مطابقة</div>`}
      </div>`;
  }

  function _renderIndex(root) {
    const records = _state.allRecords || [];
    const all = _buildSubjectIndex(records);
    const q = (_state.indexQuery||"").trim();
    let list = q ? all.filter(g=>g.name.includes(q)) : all.slice();
    list = _sortIndexList(list, _state.indexSort||"name");
      ...
      <div class="grades-sheet">
        <div class="grades-sheet-header">
          ...
          <div class="grades-sheet-title">
            <i class="fa-solid fa-arrow-down-a-z"></i> سجل المواد الشامل
          </div>
        </div>
        <div class="grades-sheet-body">
          ${all.length===0 ? `
            <div class="grades-empty-state">
              <div class="grades-empty-icon"><i class="fa-solid fa-book"></i></div>
              <div class="grades-empty-title">لا توجد مواد مسجلة بعد</div>
              <div class="grades-empty-sub">أضف سجلاتك الأكاديمية أولاً</div>
            </div>
          ` : `
          <div class="gr-idx-search-wrap">
            <input type="text" class="grades-input" placeholder="ابحث عن اسم مادة..."
              value="${_escAttr(_state.indexQuery||"")}"
              oninput="window.GradesModule._filterIndex(this.value)">
            <i class="fa-solid fa-magnifying-glass"></i>
          </div>
          ${_idxSortChipsHtml()}
          <div id="gr-idx-results">${_idxListHtml(list, all.length)}</div>
          `}
        </div>
      </div>
      ${_refModalHtml()}
      ...
      <div class="grades-sheet">
        ...
        <div class="grades-sheet-body">
          <div class="gr-home-actions">
            ...
            <button class="gr-btn-acadstats" title="سجل المواد الشامل"
                onclick="window.GradesModule._goIndex()">
  ...
  const GradesModule = {
    ...
    _goIndex: function(){
      _state.screen="index"; _state.indexQuery=_state.indexQuery||"";
      const root=_root();
      if(_state.allRecords===null){
        _renderLoading(root,"جارٍ تحميل السجل...");
        _loadAll().then(recs=>{ _state.allRecords=recs; if(_state.screen==="index") _renderIndex(_root()); });
      } else { _renderIndex(root); }
    },

    _filterIndex: function(val){
      _state.indexQuery = val;
      const all = _buildSubjectIndex(_state.allRecords||[]);
      const q = val.trim();
      let list = q ? all.filter(g=>g.name.includes(q)) : all.slice();
      list = _sortIndexList(list, _state.indexSort||"name");
      const el = document.getElementById("gr-idx-results");
      if(el) el.innerHTML = _idxListHtml(list, all.length);
    },

    _setIndexSort: function(mode, btn){
      _state.indexSort = mode;
      if(btn){
        const group = btn.closest(".grades-chips");
        if(group) group.querySelectorAll(".grades-chip").forEach(c=>c.classList.remove("active"));
        btn.classList.add("active");
      }
      const all = _buildSubjectIndex(_state.allRecords||[]);
      const q = (_state.indexQuery||"").trim();
      let list = q ? all.filter(g=>g.name.includes(q)) : all.slice();
      list = _sortIndexList(list, mode);
      const el = document.getElementById("gr-idx-results");
      if(el) el.innerHTML = _idxListHtml(list, all.length);
    },
