...
function _buildPollHTML(docId, poll, myUid) {
  ...
  const optsHTML = poll.options.map((opt, i) => {
    ...
    return `<button class="poll-option${voted?" voted":""}${isMyVote?" my-vote":""}"
      onclick="castPollVote('${window.esc(docId)}',${i})"
      ...
      <div class="poll-option-inner">
        <div class="poll-opt-left">
          ...
          <span class="poll-opt-label">${window.esc(opt)}</span>
        </div>
  ...
  return `<div class="poll-card" data-poll-id="${window.esc(docId)}">
    <div class="poll-question"><i class="fa-solid fa-chart-bar"></i>${window.esc(poll.question)}</div>
...
function _refreshPollCard(docId, poll) {
  ...
  if (!card) return;
  const newHTML = _buildPollHTML(docId, poll, window.currentUser?.uid);
...
window.castPollVote = async function(docId, optionIndex) {
  ...
  const msgRef  = doc(window.db, colPath, docId);
  try {
...
const _origStartChat = typeof window.startChatListener === "function" ? window.startChatListener : null;
...
window.pollAddOption = function() {
  ...
  if (count >= 6) { window.toast("الحد الأقصى 6 خيارات","warn"); return; }
...
async function ctxReact(emoji) {
  ...
  if (!window._ctxDocId || !window.currentUser) return;
  await toggleReaction(window._ctxDocId, emoji, window._currentChatId);
}
...
async function toggleReaction(docId, emoji, chatId) {
  ...
  const colPath = window.chatColPath(chatId || window._currentChatId);
  const msgRef  = doc(window.db, colPath, docId);
  try {
    ...
    await updateDoc(msgRef, upd);
  } catch(e) { console.warn("reaction error:", e); window.toast("فشل التفاعل: " + (e.code || e.message || "خطأ غير معروف"), "error"); }
}
...
function _updateReactionChips(row, docId, reactions) {
  ...
  if (html) {
    ...
    newEl.querySelectorAll(".reaction-chip").forEach(chip => {
      ...
      chip.onclick = () => toggleReaction(docId, emoji, window._currentChatId);
    });
...
function ctxEdit() {
  window.hideMsgCtxMenu();
  if (!window._ctxDocId || !window._ctxData) return;
  if (window._ctxData.uid !== window.currentUser?.uid) return;
  if (!window._ctxData.text) return;
  _editDocId = window._ctxDocId;
  _editOrigText = window._ctxData.text;
...
async function _saveEdit() {
  if (!_editDocId || !window.currentUser) return;
  const input = document.getElementById("chatInput");
  const newText = input ? input.value.trim() : "";
  if (!newText) { window.toast("لا يمكن حفظ رسالة فارغة", "warn"); return; }
  if (newText === _editOrigText) { _hideEditBar(); return; }
  try {
    const colPath = window.chatColPath(window._currentChatId);
    await updateDoc(doc(window.db, colPath, _editDocId), {
      text: newText,
      edited: true,
      editedAt: serverTimestamp()
    });
    _hideEditBar();
    window.toast("تم تعديل الرسالة", "success");
  } catch(e) {
    window.toast("فشل تعديل الرسالة", "error");
    console.error(e);
  }
}
...
document.addEventListener("click", (e) => {
  ...
  if (menu && !menu.contains(e.target)) window.hideMsgCtxMenu();
});
document.addEventListener("scroll", window.hideMsgCtxMenu, true);
window.addEventListener("popstate", window.hideMsgCtxMenu);
  ...
  container.addEventListener("touchmove", e => {
    ...
    if (!_triggered && dist >= TRIGGER) {
      ...
      try {
        ...
        window.setReply({ docId: _row.dataset.swipeDocId, ...d });
      } catch(err) {}
    }
  }, { passive: true });
