...
function openReportModal(ctx) {
  if (!window.currentUser?.uid) { window.toast("يجب تسجيل الدخول أولاً", "error"); return; }
  if (ctx.targetUid === window.currentUser.uid) { window.toast("لا يمكنك التبليغ عن نفسك", "warn"); return; }
  _rptContext = ctx;
  ...
  if (tbox) {
    ...
    if (ctx.type === "message") {
      tbox.innerHTML = `<div class="rpt-target-name"><i class="fa-solid fa-message" style="color:var(--muted);margin-left:6px;"></i>${window.esc(ctx.targetName)}</div>${ctx.msgText ? `<div class="rpt-target-msg">${window.esc(ctx.msgText)}</div>` : ""}`;
    } else {
      tbox.innerHTML = `<div class="rpt-target-name"><i class="fa-solid fa-user" style="color:var(--muted);margin-left:6px;"></i>${window.esc(ctx.targetName)}</div>`;
    }
  }
...
async function submitReport() {
  if (!window.currentUser?.uid || !_rptContext || !_rptSelectedReason) return;
  // Anti-spam cooldown
  const now = Date.now();
  if (now - _rptLastTime < RPT_COOLDOWN_MS) {
    window.toast("يرجى الانتظار قبل إرسال بلاغ آخر", "warn"); return;
  }
  const submitBtn = document.getElementById("rptSubmitBtn");
  if (submitBtn) submitBtn.disabled = true;
  ...
  if (reason === "other") {
    ...
    if (!txt) { window.toast("يرجى كتابة سبب التبليغ", "warn"); if (submitBtn) submitBtn.disabled = false; return; }
    reason = txt;
  }
  ...
  if (_rptContext.msgId) {
    try {
      const dupQ = query(
        ...
        where("reporterUid",  "==", window.currentUser.uid),
        where("msgId",        "==", _rptContext.msgId),
        limit(1)
      );
      const dupSnap = await getDocs(dupQ);
      if (!dupSnap.empty) {
        window.toast("لقد أرسلت بلاغاً على هذه الرسالة من قبل", "warn");
        if (submitBtn) submitBtn.disabled = false;
        return;
      }
    } catch(_) {}
  ...
  try {
    const reportData = {
      ...
      reporterName:   window.currentName || window.currentUser.email || "مجهول",
      ...
      chatId:         _rptContext.chatId   || window._currentChatId || null,
      reason:         reason,
    ...
    window.toast("تم إرسال البلاغ بنجاح", "success");
    document.getElementById("reportModalBackdrop").classList.remove("show");
    document.body.style.overflow = "";
...
function ctxReport() {
  ...
  if (!window._ctxData || !window._ctxDocId) return;
  const targetUid  = window._ctxData.uid || window._ctxData.senderId || null;
  const targetName = window._ctxData.name || window._ctxData.senderName || "مستخدم";
  const msgText    = window._ctxData.text || null;
  openReportModal({
    type:       "message",
    targetUid:  targetUid,
    targetName: targetName,
    msgId:      window._ctxDocId,
    msgText:    msgText,
    chatId:     window._currentChatId || null,
  });
}
...
function _renderOwnerReports() {
  ...
  listEl.innerHTML = data.map(r => {
    ...
    const msgBox = r.msgText ? `<div class="rpt-rc-msg-box"><i class="fa-solid fa-quote-right" style="color:var(--muted);margin-left:6px;"></i>${window.esc(r.msgText)}</div>` : "";
    const hasMsgDelete = r.msgId && r.chatId;
    const delBtnHtml = hasMsgDelete
      ? `<button class="rpt-act-btn del-msg" onclick="ownerDeleteReportedMsg('${window.esc(r.id)}','${window.esc(r.chatId)}','${window.esc(r.msgId)}')"><i class="fa-solid fa-trash"></i> حذف الرسالة</button>`
      : "";
    return `
    <div class="rpt-report-card status-${r.status}" id="rptCard-${r.id}">
      <div class="rpt-rc-hdr">
        ...
        <span class="rpt-rc-reason">${window.esc(r.reason)}</span>
        <span class="rpt-rc-date">${date}</span>
      </div>
      <div class="rpt-rc-row">
        <div class="rpt-rc-person">
          <div class="rpt-rc-person-lbl">المبلغ</div>
          <div class="rpt-rc-person-name">${window.esc(r.reporterName || "—")}</div>
          <div class="rpt-rc-person-uid">${window.esc(r.reporterUid || "")}</div>
        </div>
        <div class="rpt-rc-person">
          <div class="rpt-rc-person-lbl">المبلغ عنه</div>
          <div class="rpt-rc-person-name">${window.esc(r.targetName || "—")}</div>
          <div class="rpt-rc-person-uid">${window.esc(r.targetUid || "")}</div>
        </div>
      </div>
      ${msgBox}
      <div class="rpt-rc-actions">
        ...
        ${delBtnHtml}
        <button class="rpt-act-btn profile"  onclick="openProfileViewModal('${window.esc(r.targetUid)}')"><i class="fa-solid fa-id-card"></i> البروفايل</button>
      </div>
    </div>`;
  }).join("");
}
...
async function updateReportStatus(reportId, status) {
  try {
    ...
    if (dEl) dEl.textContent = resolved;
    if (rEl) rEl.textContent = rejected;
    window.toast("تم تحديث حالة البلاغ", "success");
  } catch(e) { window.toast("فشل تحديث الحالة", "error"); }
}
...
async function ownerDeleteReportedMsg(reportId, chatId, msgId) {
  ...
  try {
    ...
    if (chatId === "public") {
      ...
      msgRef = doc(window.db, "privateChats", chatId, "messages", msgId);
    } else {
      msgRef = doc(window.db, "rooms", chatId, "messages", msgId);
    }
    ...
    await updateDoc(doc(window.db, "reports", reportId), { status: "resolved" });
    const idx = _ownerReportsAll.findIndex(r => r.id === reportId);
    if (idx !== -1) _ownerReportsAll[idx].status = "resolved";
    _renderOwnerReports();
    window.toast("تم حذف الرسالة وإغلاق البلاغ", "success");
  } catch(e) { window.toast("فشل حذف الرسالة", "error"); }
}
...
function _renderAdminReports() {
  ...
  listEl.innerHTML = data.map(r => {
    ...
    try { const ts = r.createdAt?.toDate?.() || new Date(r.createdAt); date = ts.toLocaleDateString("ar-EG", { day:"numeric", month:"short", year:"numeric" }); } catch(_) {}
    const msgBox = r.msgText ? `<div class="rpt-rc-msg-box"><i class="fa-solid fa-quote-right" style="color:var(--muted);margin-left:6px;"></i>${window.esc(r.msgText)}</div>` : "";
    // الأدمن يستطيع حذف الرسالة إذا كانت صلاحياته تسمح (window.isAdmin)
    const hasMsgDelete = r.msgId && r.chatId;
    const delBtnHtml = hasMsgDelete
      ? `<button class="rpt-act-btn del-msg" onclick="adminDeleteReportedMsg('${window.esc(r.id)}','${window.esc(r.chatId)}','${window.esc(r.msgId)}')"><i class="fa-solid fa-trash"></i> حذف الرسالة</button>`
      : "";
    return `
