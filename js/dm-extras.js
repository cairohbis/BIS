...
window._dmExtrasBindLongPress = function () {
  // كل حاويات قوائم المحادثات المعروفة (القائمة الرئيسية + شاشة الأرشيف
  // المستقلة) — بدون هذا التعميم، لن يعمل الضغط المطول داخل شاشة الأرشيف
  // إطلاقًا، وبالتالي لن يكون هناك أي طريقة لإلغاء الأرشفة بعد تفعيلها.
  ["dmsConvList", "dmsArchiveList"].forEach(_dmExtrasBindLongPressTo);
};
function _dmExtrasBindLongPressTo(containerId) {
...
window._dmExtrasOpenMenu = function (otherId) {
  ...
  const searchBar = document.querySelector("#page-dms .dms-search-bar");
...
(function _exInjectStyles() {
  ...
  style.textContent = `
    ...
    .dmex-pill i { color: rgba(224,178,60,.55); font-size: 12.5px; }
    .dmex-pill.active { background: rgba(224,178,60,.08); border-color: rgba(224,178,60,.18); color: rgba(224,178,60,.75); }
    .dmex-pill.active i { color: rgba(224,178,60,.75); }
